...
class AuthCodeControllerTest {
    ...
    private static final String BASE_URL = "/api/v1/auth/code";
    ...
    class Exists {
        ...
        void DB에_이메일이_존재하지_않을_경우_FALSE를_응답하다() throws Exception {
            ...
            resultActions.andExpect(status().isOk())
                .andExpect(content().json(objectMapper.writeValueAsString(new AuthCodeExistsEmailResponseView(email, false))));
        }
        ...
        void DB에_이메일이_존재할_경우_TRUE를_응답한다() throws Exception {
            ...
            // then
            resultActions.andExpect(status().isOk())
                .andExpect(content().json(objectMapper.writeValueAsString(new AuthCodeExistsEmailResponseView(email, true))));
        }
    }
    ...
    class SendCode {
        ...
        void 인증_코드를_성공적으로_발생했다면_사용자에게_인증_코드를_전달해준다() throws Exception {
            ...
            // then
            resultActions.andExpect(status().isOk())
                .andExpect(content().json(objectMapper.writeValueAsString(AuthCodeSendCodeResponseView.createMessage(certTarget, 10))));
        }
    }
    ...
    class ValidateCode {
        ...
        void 인증_코드를_성공적으로_인증할_경우_코드_인증에_성공했다는_응답을_전달해준다() throws Exception {
            ...
            AuthCode authCode = AuthCodeFixture.createAuthCode(email, code, ttl);
            ...
            // then
            resultActions.andExpect(status().isOk())
                .andExpect(content().json(objectMapper.writeValueAsString(new AuthCodeValidateCodeResponseView(code, true))));
        }
        ...
        void 만료_기간이_지난_인증_코드를_인증하려고_시도할_경우_실패했다는_응답을_전달한다() throws Exception {
            ...
            AuthCode expiredAuthCode = AuthCodeFixture.createAuthCode(email, code, ttl);
            authCodeRepository.save(expiredAuthCode);
            Thread.sleep(500);
            ...
            // then
            resultActions.andExpect(status().isOk())
                .andExpect(content().json(objectMapper.writeValueAsString(new AuthCodeValidateCodeResponseView(code, false))));
        }
